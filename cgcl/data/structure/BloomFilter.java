...
/*
 * 构建一个bloomfilter，首先需要知道key有多少个，然后是每个key需要多少给bucket，或者最小的出错概率是多少
 * 然后计算出需要的bit个数，以及hash函数的个数
 */
public class BloomFilter extends Filter
{
    ...
    public static BloomFilter getFilter(long numElements, int targetBucketsPerElem)
    {
        ...
        {
         assert(bucketsPerElement >= targetBucketsPerElem);
        }
        BloomCalculations.BloomSpecification spec = BloomCalculations.computeBloomSpec(
        		bucketsPerElement);
